...
import { removeAllExtraHearts, getExtraHearts, setExtraHearts } from "../hearts/extraHearts.js";
...
export function handlePlayerDeath(event) {
  ...
  try {
    ...
    // Remove one extra heart (not all)
    const currentHearts = getExtraHearts(player);
    if (currentHearts > 0) {
      setExtraHearts(player, currentHearts - 1);
      system.runTimeout(() => {
        player.sendMessage(`${CONFIG.COLORS.WARNING}You lost 1 extra heart! (${currentHearts - 1}/${CONFIG.MAX_EXTRA_HEARTS} remaining)`);
      }, 20);
    }
